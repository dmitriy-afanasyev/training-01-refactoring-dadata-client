...
**Контракт: null или исключение — осознанный выбор.**

Метод бросает исключение если вызывающий код не может продолжить без результата (`findByInn` → `PartyNotFoundException`).

`null` допустим если отсутствие результата — ожидаемое состояние (`findById(int $id): ?Party`).
...
### 3.1 Почему интерфейсы репозиториев находятся в слое Domain?

- **Репозиторий — это концепция предметной области.** Его методы (например, `FindActiveCustomers()`) выражают бизнес-намерения, а не технические операции — он является частью универсального языка (Ubiquitous Language).

- **Принцип инверсии зависимостей (DIP).** Абстракция (интерфейс) принадлежит высокоуровневому модулю (Domain), а реализация — низкоуровневому (Infrastructure). Это обеспечивает независимость домена от деталей хранения.

- **Домен не должен зависеть от Application.** Хотя репозиторий часто используется в прикладном слое, его интерфейс определяется в домене, **чтобы доменные сервисы** тоже могли его использовать, а Application просто реализует свои сценарии, опираясь на эту абстракцию.

- **Защита модели.** Интерфейс, определённый в Application, мог бы возвращать DTO или другие «прикладные» объекты, что загрязнило бы доменную модель. Определяя интерфейс в Domain, мы гарантируем, что он работает с сущностями и value-объектами.

**Итого:** Интерфейс репозитория принадлежит доменному слою, его конкретная реализация — инфраструктурному. Это стандартная практика DDD и чистой архитектуры.
