...
(Варианты перевода: Операционный уровень, уровень Прикладных операций, уровень Вариантов использования)

## Назначение

Application — это **уровень Use Cases** (сценариев использования). Он оркестрирует выполнение бизнес-кейсов: координирует доменные объекты, управляет транзакциями, кэшированием и трансформацией данных между слоями.
...
Infrastructure может зависеть и от Application — когда Операционный уровень определяет **интерфейсы сервисов** (outbound ports), а Infrastructure их реализует. Например, уведомление об обновлении кэша:
...
### Application Service (служба прикладного уровня) — оркестратор, не доменная логика
...
> **Примечание:** DTO могут использовать доменные enum — они описывают тот же язык, что и домен, но не несут поведения. Это допустимо, потому что enum — это данные, не логика.
> Однако если Enum содержит бизнес-логику, то это уже Value Object и в этом случае он не должен попадать в слой Presentation.
> Сейчас оба класса `PartyStatus`, `BankStatus` имеют метод `isActive()` - это бизнес-логика.
> Соответственно не должны попадать в Presentation
...
- Прикладной уровень решает: этот сценарий стоит кешировать, этот — нет
...
Application-уровень определяет где начинается и заканчивается транзакция, потому что только он знает что такое «единица работы». Домен оперирует агрегатами, не знает про БД.
...
### 2. Кэширование — на прикладном уровне
...
Границы транзакции определяет Application-уровень, потому что он знает что такое «единица работы». Домен оперирует агрегатами и не должен знать про БД. См. раздел «Транзакции» выше.
...
### 6. Value Objects создаются на прикладном уровне

Валидация входных данных (`Inn::fromString()`, `Bic::fromString()`) происходит в сервисе перед вызовом репозитория. Доменный уровень получает уже валидные VO.
